...
var UCI = function () {
    ...
    findExecutableFilesIn(findAllFilesIn(engineRoot), function (executableFiles) {
    ...
    function isExecutableMimeType(mimeType) {
        ...
        else if (os.platform() === 'win32' && mimeType === 'application/x-dosexec') {
        }
        return false;
    }
    ...
    function raiseGameendsEvent(timeup) {
        if (timeup) {
            ...
            var result = (self.chess.turn() === 'w' ? '0-1' : '1-0');
            var reason = (self.chess.turn() === 'w' ? "white" : "black") + ' is checkmated';
            self.emit('gameends', result, reason);
        }
        else if (self.chess.in_stalemate()) {
            var result = '1-1';
            var reason = (self.chess.turn() === 'w' ? "white" : "black") + ' is stalemated';
            self.emit('gameends', result, reason);
        }
        else if (self.chess.in_threefold_repetition()) {
            var result = '1-1';
            var reason = 'draw due to threefold repetition';
            self.emit('gameends', result, reason);
        }
		else if (self.chess.insufficient_material()) {
			var result = '1-1';
            var reason = 'draw due to insufficient material';
            self.emit('gameends', result, reason);
		}
        else if (self.chess.in_draw()) {
            var result = '1-1';
            var reason = 'game is a draw';
            self.emit('gameends', result, reason);
        }
    }

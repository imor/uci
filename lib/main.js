...
var UCI = function () {
    ...
    findUciEnginesAmong(utilities.findAllFilesIn(enginesRoot)).then( function (engineProxies) {
...
function findUciEnginesAmong(files) {
    ...
    function emitReadyEvent() {
        ...
        if (resolved === files.length) {
            if (Object.keys(engineProxies).length > 0) {
                deferred.resolve(engineProxies);
            } else {
                deferred.reject('No engine found.');
            }
        }
    }
    ...
    function engineStartedHandler(engineObj) {
        ...
        engineObj.stop().then(emitReadyEvent).fail(engineStartupFailureHandler).done();
    }

    function engineStartupFailureHandler(error) {
        emitReadyEvent();
    }
    ...
    for (var i = 0;i < files.length;i++) {
        ...
        engine.start().then(engineStartedHandler).fail(engineStartupFailureHandler).done();
    }

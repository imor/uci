...
function isExecutableMimeType(mimeType) {
    if ((os.platform() === 'linux' && mimeType === 'application/x-executable') ||
            ...
            (os.platform() === 'darwin' && mimeType === 'application/octet-stream')) {
        return true;
    }
    return false;
}
...
Engine.prototype.start = function () {
    ...
    mime(self.engineExecutablePath, function (err, type) {
        if (!err && isExecutableMimeType(type)) {
            ...
            uciCommand.run(self.engineProcess).then(function (idAndOptions) {
                self.engineProxy = {};
                self.engineProxy.name = idAndOptions.id.name;
                self.engineProxy.author = idAndOptions.id.author;
                self.engineProxy.options = idAndOptions.options;
                self.engineProxy.executablePath = self.engineExecutablePath;
                self.emit('EngineStarted', self);
            }, function (error) {
                self.emit('Error', error);
            });
        } else {
            self.emit('Error', self.engineExecutablePath + ' is not executable');
        }
    });
};
